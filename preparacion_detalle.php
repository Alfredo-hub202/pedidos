...
//echo debug('w-preparacion-detalle.pro',$VARS);
  ...
  <link rel="stylesheet" type="text" href="./storage.js">
      ...
      <table class="table table-striped">
        ...
        <tbody>
          <tr>
            ...
            <td>
              <?php echo utf8_decode($xml->pedido);?>
            </td>
            <td>
              <?php echo utf8_decode($xml->su_pedido);?>
            </td>
            <td>
              <?php echo utf8_decode($xml->centro_fiscal);?>
            </td>
          </tr>
          <tr>
          ...
          <tr>
            <th colspan="2">
              <?php echo strtoupper($xml->almacen)?>
            </th>
            <td colspan="2">
              <?php echo utf8_decode($xml->direccion);?>
            </td>
          </tr>
          ...
          <td>
            <?php echo fecha($xml->fecha);?>
          </td>
          <td>
            <?php echo fecha($xml->fecha_entrega);?>
          </td>
          <td>
            <?php echo fecha($xml->total_lin);?>
          </td>
          <td>
            <?php echo fecha($xml->obser);?>
          </td>
          </tr>
        </tbody>
      </table>
...
<br>
        ...
        <tbody>
          ...
          <tr>
            <form action="<?php $_SERVER['PHP_SELF']?>" method="post" autocomplete="off">
              ...
              <td><strong>
                  <?php echo '<font color=" '.$linea->color .'">'. $linea->cod_art.'</font>'?>
                </strong> <br />
                <?php echo '<font color=" '.$linea->color .'">'. $linea->nombre.'</font>'?>
              </td>
              <td>
                <?php echo '<font color=" '.$linea->color .'">'. $linea->ean.'</font>'?>
              </td>

              <td>
                <?php echo '<font color=" '.$linea->color .'">'. $linea->cod_lote.'</font>'?>
              </td>
              <td>
                <?php echo '<font color=" '.$linea->color .'">' . number_format(strval($linea->total_a_prep),0,",",".").'</font>'?>
              </td>
              <td>
                <?php echo '<font color=" '.$linea->color .'">'  . number_format(strval($linea->total_prep_pistola),0,",",".").'</font>'?>
            ...
            if (confirm("Borrar Linea?")){
              ...
              <td>
                <?php echo '<font color=" '.$linea->color .'">' . number_format(strval($linea->total_pte_pistola),0,",",".").'</font>'?>

              <td>
                <?php echo '<font color=" '.$linea->color .'">'. $linea->pto_operacional_nombre.'</font>'?>
              </td>
              <td>
                ...
                <input type="hidden" name="W_PREPARACION" id="W_PREPARACION"
                  value="<?php echo $linea->preparacion;?>" />
                <input type="hidden" name="W_PREPARACION_LIN" id="W_PREPARACION_LIN"
                  value="<?php echo $linea->preparacion_lin;?>" />
                <input type="hidden" name="W_PREPARACION_LOTE" id="W_PREPARACION_LOTE"
                  value="<?php echo $linea->preparacion_lote;?>" />
                <input type="hidden" name="W_CNT" id="W_CNT" value="1" /><br /><br />
                <?php	if (strval($linea->total_prep_pistola) == $linea->total_a_prep){ ?>
                  <div class="progress border">

  <div class="progress-bar" role="progressbar" style="width: 100%;" aria-valuenow="25" aria-valuemin="0" aria-valuemax="100" >100%</div>
</div>
                  <!-- <input type="image" style="border:0" title="Completado" src="images/borrar.png" alt="Borrar" align="bottom"> -->
                <?php
        ...
        <tfoot>
            <tr>
                ...
                <th></th>
                <th></th>
                <th><button type="button" class="btn btn-primary">Guardar</button>
</th>
            </tr>
      </table>
  ...
  <script>
      ...
      $('#example').DataTable({
        initComplete: function () {
          this.api().columns([5, 6, 7]).every(function (i) {
            var column = this,
              select = $('<select style="width:200px"><option value="">Seleccione un Centro</option></select>')
                .appendTo($('#search' + i))
                .on('change', function () {
    ...
    //   sessionStorage.setItem(producto, precio);
